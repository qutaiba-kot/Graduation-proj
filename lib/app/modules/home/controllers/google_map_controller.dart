...
class MapController extends GetxController {
  ...
  double currentZoom = 15.0; // مستوى التكبير الابتدائي
  ...
  try {
    ...
    if (permission == LocationPermission.deniedForever) {
      throw Exception("⛔ إذن الموقع مرفوض بشكل دائم. لا يمكن طلب الإذن مجددًا.");
    }
...
// قراءة الوضع الحالي من التخزين المحلي
...
Future<void> loadAndApplyMapStyle() async {
  if (_googleMapController == null) {
    print("⚠️ [WARNING] GoogleMapController is not initialized yet. Skipping style application.");
    return;
  }
  ...
  try {
    ...
    print("🎭 [INFO] Applying map style: ${isDarkMode.value ? "Night" : "Day"} mode.");
...
Future<Map<String, dynamic>> loadMapStyles() async {
  ...
  try {
    final String jsonString = await rootBundle.loadString('lib/app/assets/map_design/map_style.json');
...
String getMapStyle(Map<String, dynamic> mapStyles, bool isDarkMode) {
  print("🎛️ [INFO] Selecting map style for ${isDarkMode ? "Night" : "Day"} mode.");
  return isDarkMode ? json.encode(mapStyles['night']) : json.encode(mapStyles['day']);
}
  ...
  final RxList<Map<String, dynamic>> searchSuggestions =
      <Map<String, dynamic>>[].obs; // اقتراحات البحث
  ...
  if (routePolyline.value.points.isEmpty || selectedDestination.value == null) {
    return;
  }
  ...
  if (totalRemainingDistance <= 5) { // إذا كانت المسافة أقل من 5 أمتار
    Get.snackbar(
      "Congratulations!".tr,
  ...
  // جلب اقتراحات البحث باستخدام Places API
  ...
  // جلب إحداثيات الموقع المختار باستخدام Details API
